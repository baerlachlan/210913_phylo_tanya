...
The resulting `FASTA` file [`proteins.fasta`]() was saved locally, ready for multiple sequence alignment.
...
The resulting alignment was [`proteins_msa.fasta`]() was saved for use with [`Gblocks` software](http://molevol.cmima.csic.es/castresana/Gblocks.html).
...
For visualisation purposes the alignment was saved in `PDF` format to file [`msa.pdf`]().
...
The resulting alignment file [`proteins_msa.fasta-gb`]() was output by the software for further phylogenetic analysis.
A visualisation of the `Gblocks` selection process was also output in the file [`proteins_msa.fasta-gb.htm`]().
